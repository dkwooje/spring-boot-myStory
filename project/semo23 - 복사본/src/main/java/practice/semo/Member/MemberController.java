...
@ResponseBody
public MemberDto getUser1(@PathVariable Long user_id) {
    var a = memberRepository.findById(user_id);
    var result = a.get();
    ...
    @ResponseBody
    public MemberDto getUser2(@PathVariable Long user_id) {
        var a = memberRepository.findById(user_id);
        var result = a.get();
        var data = new MemberDto(result.getUsername(), result.getDisplayName(), result.getUser_id());
        return data;
    }
